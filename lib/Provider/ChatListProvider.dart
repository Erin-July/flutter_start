...
import 'package:startup_namer/Model/ChatContentModel.dart';
...
class ChatListProvider with ChangeNotifier{
  ...
  // get newToken => null;
  // List<ChatContentModel> chats = <ChatContentModel>[];
  initFriends()   {
    // var httpClient = new HttpClient();
    // var url = new Uri.http('175.27.189.9', '/contacts/getList', {
    //   'id': _phoneNumber,
    ...
    //   var resultUsers = data['Contact'];
    //   var dio = DioHelper().dio;
    //   var resultUsers = await dio.get('/contacts/getList');
    //     resultUsers.
      data.forEach((item) {
        users.add(UserModel(
      ...
      // users = users.getRange(5,50).toList();


      // users.forEach((item) {
      //   List<UserModel> userIds = <UserModel>[];
      //   userIds.add(item);
      //   // chats.add(ChatContentModel(
      //   //   ifRead: false,
      //   //   lastContent: "这是我发来的最后一条信息",
    ...
    List data = [
      ...
      },
      {
        ...
        "avatar_url": "https://raw.githubusercontent.com/flutter/website/master/_includes/code/layout/lakes/images/lake.jpg",
